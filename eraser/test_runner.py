...
def run_tests():
    ...
    assert res3.bytes_processed > 0, (
        "Folder byte accounting should reflect actual file contents"
    )
    ...
    try:
        eraser.sanitize(
            job_id="TEST-SAFETY-02", target_path="./sneaky_link", method="NIST_CLEAR"
        )
        raise AssertionError(
            "Expected PermissionError for a symlink pointing into a protected path"
        )
    except PermissionError as e:
    ...
    try:
        eraser.sanitize(
            job_id="TEST-METHOD-01",
            target_path="./test_drive3.raw",
