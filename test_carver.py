...
class TestForensicNexus(unittest.TestCase):
    ...
    def test_carver_execution_and_metrics(self):
        res = self.carver.carve_target(
            job_id="TEST-001", target_path=self.test_file, output_base_dir=self.test_dir
        )
    ...
    def test_eraser_execution_and_signature(self):
        res = self.eraser.sanitize(
            target_path=self.test_file, method="NIST_CLEAR", job_id="TEST-ERASE-001"
        )
