...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "eraser"))
)
...
def generate_seized_evidence_image(image_path: str):
    ...
    with zipfile.ZipFile(docx_buf, "w") as zf:
        ...
        zf.writestr(
            "word/document.xml",
            "<w:document><w:body><w:p>Seized Data</w:p></w:body></w:document>",
        )
...
def run_tests():
    ...
    res = carver.carve_target(
        job_id="CASE-SEIZED-01", target_path=test_img, output_base_dir="./test_cases"
    )

    print(
        f"Status: Success | Recovered Deleted Files: {res['deleted_files_recovered']}"
    )
    print(
        f"Entropy Analysis Exhaustion: {res['entropy_analysis']['carving_exhaustion_confidence_pct']}%"
    )

    for f in res["carved_catalog"]:
        print(
            f" -> [{f['classification']}] {f['file_name']} ({f['file_type']}) | Ext: .{f['extension']} | Score: {f['confidence_score']}% | Space: {f['source_space']}"
        )
    ...
    post_wipe_res = carver.carve_target(
        job_id="CASE-POST-WIPE-01", target_path=test_img, output_base_dir="./test_cases"
    )
    print(f"Post-Sanitization Files Carved: {post_wipe_res['deleted_files_recovered']}")
    print(
        f"Sanitized Zeroed Sectors: {post_wipe_res['entropy_analysis']['zeroed_blocks']} blocks"
    )

    assert post_wipe_res["deleted_files_recovered"] == 0, (
        "Security Failure: Files recovered from sanitized target"
    )
