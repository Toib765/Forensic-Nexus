...
class FatExtractor:
    ...
    def parse_boot_sector(self) -> bool:
        try:
            with open(self.target_path, "rb") as f:
                ...
                fat_size_32 = (
                    struct.unpack_from("<I", bs, 36)[0] if len(bs) >= 40 else 0
                )
                self.fat_size = fat_size_16 if fat_size_16 != 0 else fat_size_32

                if (
                    self.bytes_per_sec == 0
                    or self.sec_per_clus == 0
                    or self.fat_size == 0
                ):
                    return False
    ...
    def get_cluster_allocation_map(
        self,
        ...
        try:
            with open(self.target_path, "rb") as f:
                ...
                for clus_idx in range(
                    2, min(self.total_clusters + 2, len(fat_table_raw) // 4)
                ):
                    entry = (
                        struct.unpack_from("<I", fat_table_raw, clus_idx * 4)[0]
                        & 0x0FFFFFFF
                    )
                    if entry == 0:
                ...
                root_offset = self.data_start_byte + (
                    (self.root_clus - 2) * cluster_size
                )
                f.seek(root_offset)
