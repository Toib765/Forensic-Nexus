...
def verify_investigator_session(authorization: str | None = Header(None)):
    ...
    if not session or session["role"] not in ["ForensicInvestigator", "Admin"]:
        raise HTTPException(
            status_code=403,
            detail="Forbidden: Operation requires ForensicInvestigator or Admin privileges.",
        )
    return session


@router.post("/carve")
async def execute_carving(
    req: CarveRequest, session: dict = Depends(verify_investigator_session)
):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Deep Carving Failed: {e!s}")
...
async def inspect_hex_bytes(
    ...
    target_file = os.path.realpath(
        os.path.join(base_cases, safe_case, "carved_evidence", safe_cat, safe_file)
    )

    if not target_file.startswith(base_cases) or not os.path.exists(target_file):
        raise HTTPException(
            status_code=404, detail="Evidence artifact not found or access denied."
        )
    ...
    for idx in range(0, len(raw_bytes), 16):
        ...
        hex_lines.append(
            {"offset": f"{idx:08X}", "hex": " ".join(hex_pairs), "ascii": ascii_chars}
        )
