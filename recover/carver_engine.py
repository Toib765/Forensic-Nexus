...
class StreamCarver:
    ...
    scan_unallocated_only: bool = False,
        ...
        with open(target_path, "rb") as f:
            ...
            total_blocks = (
                math.ceil(target_bytes / self.block_size) if target_bytes > 0 else 0
            )
            ...
            while i < target_bytes - 16:
                ...
                if mm[i : i + 8] == b"\x89PNG\r\n\x1a\n":
                    iend = mm.find(
                        b"IEND", i, min(i + self.max_carve_size, target_bytes)
                    )
                    if iend != -1:
                        f_size = (iend + 8) - i
                        f_type, ext, category, confidence, found = (
                            "PNG Image",
                            "png",
                            "Images",
                            100,
                    ...
                    eoi = mm.find(
                        b"\xff\xd9", i + 2, min(i + self.max_carve_size, target_bytes)
                    )
                    if eoi != -1:
                        f_size = (eoi + 2) - i
                        f_type, ext, category, confidence, found = (
                    ...
                    eof_idx = mm.find(
                        b"%%EOF", i, min(i + self.max_carve_size, target_bytes)
                    )
                    if eof_idx != -1:
                        f_size = (eof_idx + 5) - i
                        f_type, ext, category, confidence, found = (
                    ...
                    eocd = mm.find(
                        b"PK\x05\x06", i, min(i + self.max_carve_size, target_bytes)
                    )
                    if eocd != -1:
                        comment_len = (
                            struct.unpack("<H", mm[eocd + 20 : eocd + 22])[0]
                            if eocd + 22 <= target_bytes
                            else 0
                        )
                    ...
                    mm[i : i + 16] == b"SQLite format 3\x00" and i + 100 <= target_bytes
                ):
                    p_size = struct.unpack(">H", mm[i + 16 : i + 18])[0]
                    p_count = struct.unpack(">I", mm[i + 28 : i + 32])[0]
                    calc_size = (p_size if p_size != 1 else 65536) * (
                        p_count if p_count > 0 else 2
                    )
                    ...
                    with open(target_path, "rb") as mp4_f:
                        ok, atom_len = AtomCarver.parse_mp4_stream(
                            mp4_f, i, max_size=self.max_carve_size
                        )
                    if atom_len > 0:
                        f_size = atom_len
